...
import { ArrowUpDown, ChevronDown, Edit, Eye } from "lucide-react";
...
export default function AdminDashboardUsersComponent({
  ...
  const handleBlockUnblock = async (id: string) => {
    const userDeleting = toast.loading("User Deleting...");
    try {
      ...
      toast.error("Something went wrong!", { id: userDeleting });
    }
  };
          ...
          <div className="flex gap-2">
            ...
            />
           {
            user.status === "ACTIVE" ? (
                ...
                onConfirm={() => handleBlockUnblock(user.id)}
                title="Are you sure?"
              />
            ) : (
                <ConfirmationBox
